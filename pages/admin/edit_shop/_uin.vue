<template>
  <div class="container mt-10 flex flex-col sm:flex-row">
    <main class="w-full sm:w-9/12 mb-10 sm:mr-4">
      <h1 class="text-4xl font-bold">Изменить магазин</h1>
      <form @submit.prevent="putData">
        ...
        <button
          type="submit"
          class="h-full px-3 bg-primary hover:bg-primary-600 transition-all duration-300 hover:transition-all hover:duration-300"
        >
          Добавить
        </button>
      </form>
    ...
    <aside class="w-full sm:w-3/12 mb-10 sm:ml-4">
...
export default {
  ...
  async asyncData({$api, route}:any) {
    const shopRes = await $api.get(`/shop/${route.params.uin}`)
  ...
  methods: {
    async putData() {
      try {
        // @ts-ignore
        await this.$api.put(`/shop/${this.shopData.uin}`, this.shopData)

        // @ts-ignore
        this.$router.push(`/shop/${this.shopData.uin}`)
      } catch (e) {
        console.log(e)
      }
    },
